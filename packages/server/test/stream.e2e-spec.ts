...
describe('BookmarkController (e2e)', () => {
  ...
  let ceramicSubscriberService: CeramicSubscriberService;
  ...
  it('[GET] /:network/streams/:streamId: Should return the stream that steamid is k3y52l7qbv1fryjn62sggjh1lpn11c56qfofzmty190d62hwk1cal1c7qc5he54ow ', async () => {
    const network = 'TESTNET';
    const streamId =
      'k3y52l7qbv1fryjn62sggjh1lpn11c56qfofzmty190d62hwk1cal1c7qc5he54ow';

    const result = await request(app.getHttpServer()).get(
      `/${network}/streams/${streamId}`,
    );

    expect(result.status).toBe(HttpStatus.OK);
    const response = result.body;
    expect(response.code).toBe(0);
    expect(response.data.streamId).toBe(streamId);
    expect(response.data.network).toBe(network);
  });

  it('[GET] /streams: Should return the streams that contains the steamid which is k3y52l7qbv1fryjn62sggjh1lpn11c56qfofzmty190d62hwk1cal1c7qc5he54ow ', async () => {
    const network = 'TESTNET';
    const streamId =
      'k3y52l7qbv1fryjn62sggjh1lpn11c56qfofzmty190d62hwk1cal1c7qc5he54ow';

    const result = await request(app.getHttpServer()).get(
      `/streams?network=${network}`,
    );

    expect(result.status).toBe(HttpStatus.OK);
    const response = result.body;
    expect(response.code).toBe(0);
    expect(response.data.streams.length).toBe(1);
    expect(response.data.streams[0].streamId).toBe(streamId);
    expect(response.data.streams[0].network).toBe(network);
  });
});
